...
function ResourcesPage() {
  ...
  return (
    <div className="container-page flex flex-col gap-10">
      ...
      <section>
        <h2 className='font-semibold text-2xl mb-5'>{page.middleSectionTitle}</h2>
      ...
      <section className="grid gap-6 md:grid-cols-3">
        {page.categories.map((category) => (
          ...
          >
            <h2 className="font-display text-xl font-semibold text-ink">{category.title}</h2>
      ...
      <section>
        <h3 className="font-display text-2xl font-semibold text-ink">{page.closingTitle}</h3>
        <p className="my-3 text-slate-700">{page.closingNote}</p>
        <button className='px-5 py-2 rounded-full bg-white text-ink border border-ink'>{page.closingCta} &rarr;</button>
      </section>
    </div>
  )
}
