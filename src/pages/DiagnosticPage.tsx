...
function DiagnosticPage() {
  ...
  const onSubmit = (values: DiagnosticForm) => {
    const total = questions.reduce((sum, q) => sum + Number(values[q.id as keyof DiagnosticForm]), 0)
    setScore(total)
    if (total >= 48) {
    ...
    <div className="container-page flex flex-col gap-10">
      <section className="rounded-[26px] bg-white px-8 py-10 shadow-soft">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          {page.meta.map((item) => (
            <div key={item} className="rounded-2xl bg-mist px-4 py-3 text-sm text-slate-700">
              {item}
            </div>
          ))}
        </div>
                ...
                >
                  ...
                  {errors[question.id as keyof DiagnosticForm] && (
                    <p className="mt-2 text-sm text-amber-700">
                      {errors[question.id as keyof DiagnosticForm]?.message as string}
                    </p>
                  )}
                </div>
              ))}
