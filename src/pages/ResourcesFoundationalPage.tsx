			...
			<section>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-5">
					{page.learningModel.map((model) => (
						<div
							key={model.title}
							className="rounded-2xl bg-white p-6 pb-10 shadow-soft"
						>
							<h4 className="font-semibold font-display text-lg text-center">
								{model.title}
							</h4>
							<div className="flex flex-col gap-3 mt-5">
								{model.items.map((item) => (
									<div
										key={item}
										className="border border-slate-800 p-5 rounded-2xl"
									>
										{item}
									</div>
								))}
